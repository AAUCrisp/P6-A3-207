...
for file in $(ls tests); do
    ...
    for node in "192.168.1.61" "192.168.1.80" "192.168.1.107" "192.168.1.251" ; do
        ...
        sshpass -p 123 ssh root@"$node" cat /tmp/P6-A3-207/include/db.db3 > testdata/"$(basename "$file" .json)"/db.db3
        sshpass -p 123 ssh root@"$node" cat /tmp/P6-A3-207/screenlog.0 > testdata/"$(basename "$file" .json)"/"$node".log
    done
    sleep 120
done