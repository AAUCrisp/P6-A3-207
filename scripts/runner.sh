...
function runCondition(){
    ...
    case "$(echo "$condition" | jq -r -c .name)" in
        'limit')
            ...
            echo "screen -L -dmS limit bash -s 'echo $password | sudo -S /tmp/P6-A3-207/scripts/conditions.sh limit --datarate=$datarate --iface=$interface'" | sshpass -p "$password" ssh "$name@$ip" 'bash -s'
            ;;
    esac
}
