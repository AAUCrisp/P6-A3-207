...
if [[ "$(ip addr show dev enp3s0)" == *"10.0.0."* ]]; then
    ...
    echo 123 | sudo -S ip addr flush dev enp3s0
    exit 3
else
    echo "setting up the ip address..."
        case $USER in
        "up1")
            echo 123 | sudo -S ip ad add 10.0.0.10/24 dev enp3s0
            echo "set ip address of $USER to 10.0.0.10"
            ;;
        "up2")
            echo 123 | sudo -S ip ad add 10.0.0.20/24 dev enp3s0
            echo "set ip address of $USER to 10.0.0.20"
            ;;
        "up3")
            echo 123 | sudo -S ip ad add 10.0.0.30/24 dev enp3s0
            echo "set ip address of $USER to 10.0.0.30"
            ;;
        "up0")
            echo 123 | sudo -S ip ad add 10.0.0.40/24 dev enp3s0
